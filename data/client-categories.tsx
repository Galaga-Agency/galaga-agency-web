import { ClientCategory } from "@/types/clients";

export const clientCategories: ClientCategory[] = [
  {
    icon: (
      <svg className="w-10 h-10 md:w-12 md:h-12" fill="currentColor" viewBox="0 0 20 20">
        <path fillRule="evenodd" d="M3 3a1 1 0 000 2v8a2 2 0 002 2h2.586l-1.293 1.293a1 1 0 101.414 1.414L10 15.414l2.293 2.293a1 1 0 001.414-1.414L12.414 15H15a2 2 0 002-2V5a1 1 0 100-2H3zm11.707 4.707a1 1 0 00-1.414-1.414L10 9.586 8.707 8.293a1 1 0 00-1.414 0l-2 2a1 1 0 101.414 1.414L8 10.414l1.293 1.293a1 1 0 001.414 0l4-4z" clipRule="evenodd" />
      </svg>
    ),
    titleKey: "about-page.clients-section.retail.title",
    descriptionKey: "about-page.clients-section.retail.description",
    color: "teal",
    accent: "azul-profundo",
    image: "/assets/img/sobre-nosotros/PYMEs-retail.png"
  },
  {
    icon: (
      <svg className="w-10 h-10 md:w-12 md:h-12" fill="currentColor" viewBox="0 0 20 20">
        <path fillRule="evenodd" d="M12.316 3.051a1 1 0 01.633 1.265l-4 12a1 1 0 11-1.898-.632l4-12a1 1 0 011.265-.633zM5.707 6.293a1 1 0 010 1.414L3.414 10l2.293 2.293a1 1 0 11-1.414 1.414l-3-3a1 1 0 010-1.414l3-3a1 1 0 011.414 0zm8.586 0a1 1 0 011.414 0l3 3a1 1 0 010 1.414l-3 3a1 1 0 11-1.414-1.414L16.586 10l-2.293-2.293a1 1 0 010-1.414z" clipRule="evenodd" />
      </svg>
    ),
    titleKey: "about-page.clients-section.growing.title",
    descriptionKey: "about-page.clients-section.growing.description",
    color: "mandarina",
    accent: "naranja-tostado",
    image: "/assets/img/sobre-nosotros/empresas-en-crecimiento.png"
  },
  {
    icon: (
      <svg className="w-10 h-10 md:w-12 md:h-12" fill="currentColor" viewBox="0 0 20 20">
        <path fillRule="evenodd" d="M11.3 1.046A1 1 0 0112 2v5h4a1 1 0 01.82 1.573l-7 10A1 1 0 018 18v-5H4a1 1 0 01-.82-1.573l7-10a1 1 0 011.12-.38z" clipRule="evenodd" />
      </svg>
    ),
    titleKey: "about-page.clients-section.innovative.title",
    descriptionKey: "about-page.clients-section.innovative.description",
    color: "violeta",
    accent: "azul-profundo",
    image: "/assets/img/sobre-nosotros/marcas-innovadores.png"
  }
];