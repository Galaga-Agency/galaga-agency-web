...
export default function LoadingScreen({ progress }: LoadingScreenProps) {
  ...
  const currentOffset = useRef(1256.64); // Full circle initially
  ...
  useEffect(() => {
    ...
    const targetOffset = 1256.64 - (1256.64 * progress) / 100;
    ...
    gaugeTween.current = gsap.to(currentOffset, {
      ...
      onUpdate: () => {
        if (circleRef.current) {
          circleRef.current.style.strokeDashoffset = currentOffset.current.toString();
        }
      }
    });
    ...
    >
      ...
      <div className="loading-elements-container">
        <div className="relative" style={{ width: "450px", height: "450px" }}>
          <svg width="450" height="450" className="absolute inset-0 transform -rotate-90">
            <defs>
              <linearGradient id="progressGradient" gradientUnits="userSpaceOnUse" x1="225" y1="25" x2="225" y2="425">
        ...
        <div className="text-center">
          <div className="flex items-center justify-center gap-4">
            {/* <p className="loading-text text-blanco text-2xl font-light">{t("loading")}</p> */}
            <span className="loading-percentage text-blanco text-2xl font-light">{progress}%</span>
          </div>
        </div>
      </div>
    </div>,
