...
export default function CaseStudiesSection() {
  ...
  const colorThemes: ('digital' | 'marketing' | 'events')[] = [
    'digital', 'marketing', 'events', 'digital', 'marketing', 'events', 'digital', 'events', 'marketing', 'events'
  ];
    ...
    <section className="case-studies-section section relative bg-gradient-to-br from-white via-hielo/30 to-white overflow-hidden">
      ...
      <div className="relative pb-12">
        <div className="absolute top-1/2 -translate-y-1/2 left-1/2 -translate-x-1/2 w-full max-w-5xl flex justify-between px-4 z-30 pointer-events-none">
          ...
          >
            <svg className="w-4 h-4 md:w-5 md:h-5 text-white group-hover:text-white transition-colors" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="carousel-3d-container flex justify-center items-center w-full" style={{ perspective: "1500px" }}>
          ...
          {casesWithColors.map((_, index) => (
            <button
              ...
              className={`w-2 h-2 rounded-full transition-all duration-300 ${
                index === currentIndex ? 'bg-teal/80 scale-125' : 'bg-teal/20 hover:bg-teal/40'
              }`}
            />
          ))}
        </div>
