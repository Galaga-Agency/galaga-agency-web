...
export default function Navbar() {
  ...
  return (
    <>
      ...
      <nav
        ref={navRef}
        className="sticky top-0 inset-x-0 bg-blanco/95 backdrop-blur-md border-b border-hielo z-[100]"
      >
                ...
                {navigationItems.map((item, index) => (
                  <li key={item.href} className="nav-item" data-index={index}>
                    ...
                    >
                      ...
                      {/* Hover background */}
                      <span className="absolute inset-0 bg-gradient-to-r from-teal/5 to-turquesa/10 rounded-lg transform scale-0 hover-bg"></span>

                      {/* Active indicator */}
                      {pathname === item.href && (
                        <span className="absolute bottom-0 left-1/2 w-8 h-0.5 bg-teal rounded-full transform -translate-x-1/2 active-indicator"></span>
                      )}
                    </Link>
                  </li>
                ))}
      ...
      <div
        className={`fixed inset-0 bg-negro/50 backdrop-blur-sm z-[99] lg:hidden mobile-backdrop ${
      ...
      <div
        ref={mobileMenuRef}
        className="fixed top-0 right-0 h-full w-80 max-w-[85vw] bg-blanco shadow-2xl z-[101] lg:hidden mobile-menu"
      >
        {/* Mobile Header */}
        <div className="flex items-center px-6 py-5 border-b border-hielo mobile-header">
                ...
                <Link
                  ...
                  className={`block px-4 py-4 text-lg font-medium rounded-xl transition-colors duration-300 relative overflow-hidden ${
                    pathname === item.href
                      ...
                      : "text-grafito hover:text-teal hover:bg-hielo"
                  }`}
                >
                  <span className="relative z-10">{t(item.labelKey)}</span>
                </Link>
        ...
        {/* Mobile Footer */}
        <div className="absolute bottom-0 left-0 right-0 px-6 py-6 border-t border-hielo bg-hielo/50 mobile-footer">
          <div className="space-y-4">
            ...
            <div className="flex justify-center space-x-6 pt-2 mobile-social">
              <Link
                ...
                className="p-3 text-grafito hover:text-teal transition-colors duration-300 rounded-xl hover:bg-blanco social-link"
              >
