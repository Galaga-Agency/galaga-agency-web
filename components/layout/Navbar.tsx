...
export default function Navbar() {
  ...
  return (
    <>
      <HamburgerToggle isOpen={isOpen} onClick={toggleMenu} isScrolled={isScrolled} />

      <nav
        ref={navRef}
        className={`fixed top-0 inset-x-0 z-[100] transition-all duration-300 ${
          isScrolled ? 'bg-blanco/95 backdrop-blur-md border-b border-hielo' : 'bg-transparent'
        }`}
      >
                ...
                {navigationItems.map((item, index) => (
                  <li key={item.href} className="nav-item" data-index={index}>
                    ...
                    >
                      ...
                      <span className={`absolute inset-0 rounded-lg transform scale-0 hover-bg transition-all duration-75 ${
                        isScrolled 
                          ? 'bg-gradient-to-r from-teal/5 to-turquesa/10' 
                          : 'bg-white/20'
                      }`}></span>
                      {pathname === item.href && (
                        <span className={`absolute inset-0 rounded-lg active-bg ${
                          isScrolled 
                            ? 'bg-gradient-to-r from-teal/5 to-turquesa/10' 
                            : 'bg-white/20'
                        }`}></span>
      ...
      >
        <div className="p-4 md:p-6 h-full flex flex-col">
          ...
          <div className="pt-4 md:pt-6 border-t border-gray-100">
            ...
            >
              <span className="text-xs text-gray-400 block">{t("nav.language")}</span>
            ...
            <div className="pb-4 border-b border-gray-100 mb-4">
              ...
              >
                <div className="w-8 h-8 bg-gray-100 rounded-lg flex items-center justify-center group-hover:bg-teal-100 transition-colors duration-200">
                  <svg className="w-4 h-4 text-gray-600 group-hover:text-teal-600" fill="currentColor" viewBox="0 0 20 20">
                    <path d="M2.003 5.884L10 9.882l7.997-3.998A2 2 0 0016 4H4a2 2 0 00-1.997 1.884z" />
                    <path d="M18 8.118l-8 4-8-4V14a2 2 0 002 2h12a2 2 0 002-2V8.118z" />
                  </svg>
                </div>
                <span className="pl-3 text-sm font-medium">info@galagaagency.com</span>
              </a>

              <div className="flex items-center gap-3 px-3">
                <span className="text-xs text-gray-400 font-medium">{t("footer.followUs")}</span>
